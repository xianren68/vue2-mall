      ...
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <!-- categoryName面包屑 -->
            ...
            <span @click="removeCateName"> x </span>
            </li>
          <!-- keyword面包屑 -->
        ...
        <div class="details clearfix">
            ...
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{active:isOne}">
                  <a  @click="sort('1')">综合<span v-if="isOne">{{isFlag}}</span></a>
                </li>
                <li :class="{active:isTwo}">
                  <a @click="sort('2')">价格<span v-if="isTwo">{{isFlag}}</span></a>
                </li>
              </ul>
            </div>
          </div>
          ...
          <Pager :PageNo="searchParams.pageNo" :PageSize="searchParams.pageSize" :Total="total" :continous="5" @getPage="getPage"></Pager>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    sort(f){
      ...
      let orignOrder = order.split(":")[0]
      let orignFlag = order.split(":")[1]
      if(f === orignOrder){
        this.searchParams.order = `${f}:${orignFlag === "desc"?"asc":"desc"}`
      }else{
        this.searchParams.order = `${f}:${'desc'}`
      }
      this.getSearch()
    },
    getPage(pageno){
